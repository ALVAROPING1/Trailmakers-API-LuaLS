...
4. After opening a lua file in the mods folder, you should be prompted to enable the addon. Select "Apply and modify settings". If you aren't prompted to enable the addon, verify that the LuaLS setting `workspace.checkThirdParty` is set to `true`
   - If the addon isn't enabled, add the path to the `library` folder within your local copy of this repository to the `Lua.workspace.library` LuaLS setting. You also need to copy all the settings within the `settings` key of `config.json` file. To enable it only on the mods workspace, configure LuaLS through a `.luarc.json` file in the mods directory
